...
**二分法复杂度**
n次二分后，区间缩小到(b - a)/2n。
给定a、b和精度要求ε，可以算出二分次数n，即满足(b - a)/2n < ε。
二分法的复杂度是O(logn)的。
...
————版本1————：当我们将区间[l, r]划分成[l, mid]和[mid + 1, r]时，其更新操作是l = mid + 1或者r = mid;（计算mid时不需要加1）
C++ 代码模板：
...
————版本2————：当我们将区间[l, r]划分成[l, mid - 1]和[mid, r]时，其更新操作是l = mid或者r = mid - 1;（此时为了防止死循环，计算mid时需要加1）
C++ 代码模板：
